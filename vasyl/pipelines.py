...
class VasylPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('vasyl.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `vasyl`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM vasyl WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO vasyl (date, title, link, content)"
